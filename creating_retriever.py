...
class user_retriever():
    ...
    def create_new_vdb(self):
        if self.type_of_retrieval == "dense":
            vs_dense = vector_store(collection_name=self.collection_name,embeddings=self.embeddings,path=self.path).vector_qdrant_dense()
        elif self.type_of_retrieval == "sparse":
            vs_dense = vector_store(collection_name=self.collection_name,embeddings=self.embeddings,path=self.path).vector_qdrant_sparse()
        else:
            vs_dense = vector_store(collection_name=self.collection_name,embeddings=self.embeddings,path=self.path).vector_qdrant_hybrid()
